/**
 * Live Module 10 verification against Supabase + dev server.
 * Run: pnpm dev then pnpm verify:module10
 */
...
const VERIFY_COMPANY_NAME = "Module 10 Verify Co";

const POSITIVE_BANNED_PATTERNS = [
...
let DEV_BASE_URL =
  process.env.MODULE10_VERIFY_BASE_URL ?? devBaseUrlFromPort(3000);
...
async function resolveDevBaseUrl(
  ...
  const explicit = process.env.MODULE10_VERIFY_BASE_URL?.trim();
  if (explicit) {
...
function hasPositiveBannedClaim(html: string): string | null {
  for (const pattern of POSITIVE_BANNED_PATTERNS) {
...
async function main() {
  console.log("Module 10 live verification");
  if (!process.env.DATABASE_URL) {
    console.error("DATABASE_URL missing");
    process.exit(1);
  }

  let module9Committed = false;
  try {
    const log = execSync("git log --oneline -10", { encoding: "utf8" });
    module9Committed = log.includes("Module 9");
  } catch {
    module9Committed = false;
  }
  record(
    0,
    "Module 9 committed on branch",
    module9Committed,
    module9Committed
      ? "Found Module 9 in recent git log"
      : "Module 9 commit not found",
  );
  ...
  if (tools.length < 1) {
    record(2, "Setup tool selections", false, "No published tools");
    process.exit(1);
  }
  ...
  record(
    2,
    "Score present for session",
    scoringResult.overallScore >= 0,
    `overallScore=${scoringResult.overallScore}`,
  );
  record(
    3,
    "Category scores present",
    scoringResult.categoryScores.length === 6,
    `count=${scoringResult.categoryScores.length}`,
  );
  record(
    4,
    "Findings present",
  ...
  record(
    6,
    "Result page HTTP 200 for valid session",
    resultsResponse.ok,
    `HTTP ${resultsResponse.status}`,
  );
  record(
    7,
    "Result page shows score and categories",
    resultsHtml.includes("Your AI governance posture result") &&
      resultsHtml.includes("Category scores"),
    "hero + category section found",
  );
  record(
    8,
    "Result page shows findings and recommendations",
    resultsHtml.includes("Key findings") &&
      resultsHtml.includes("Recommended next steps") &&
      resultsHtml.includes("Implementation steps"),
    "findings + recommendations sections found",
  );
  record(
    9,
    "No email capture field",
    !resultsHtml.includes('type="email"') &&
      !resultsHtml.includes("Email report") &&
      !resultsHtml.includes("email report"),
    "no email input or email report CTA",
  );
  record(
    10,
    "No PDF/report download CTA",
    !/download pdf|pdf report|download report/i.test(resultsHtml),
    "no PDF/download CTA found",
  );
  record(
    11,
    "No sales/demo CTA",
    !/book a demo|calendly|contact sales/i.test(resultsHtml),
    "no sales CTA found",
  );

  const banned = hasPositiveBannedClaim(resultsHtml);
  record(
    12,
    "Banned positive compliance claims absent",
    banned === null,
    banned ?? "none found",
  );

  const [leadRows, reportRows, ctaRows] = await Promise.all([
  ...
  const missingResponse = await fetch(
    `${DEV_BASE_URL}/ai-risk-assessment/results`,
  );
  const missingHtml = await missingResponse.text();
  record(
    16,
    "Missing session shows safe error",
    missingResponse.ok && missingHtml.includes("Result not available"),
    `HTTP ${missingResponse.status}`,
  );

  const invalidResponse = await fetch(
    `${DEV_BASE_URL}/ai-risk-assessment/results?session=not-a-valid-token`,
  );
  const invalidHtml = await invalidResponse.text();
  record(
    17,
    "Invalid session shows safe error",
    invalidResponse.ok && invalidHtml.includes("Result not available"),
